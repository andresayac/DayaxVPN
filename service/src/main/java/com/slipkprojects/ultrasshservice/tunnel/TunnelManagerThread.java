...
ServerHostKeyVerifier, DebugLogger
{
	...
	public void run()
	{
		...
		while (!mStopping) {
			try {
				...
				SkStatus.logError("<font color='red'><strong>" + mContext.getString(R.string.state_disconnected) + "</font></strong>");
				closeSSH();
	...
	protected void startClienteSSH() throws Exception {
		...
		try {
			...
			SkStatus.logInfo("<font color='green'><strong>" + mContext.getString(R.string.state_connected) + "</font></strong>");
	...
	protected void autenticar(String usuario, String senha, String keyPath) throws IOException {
		...
		try {
			if (mConnection.isAuthMethodAvailable(usuario,
				...
				SkStatus.logInfo("Authenticated with password");
		...
		try {
			if (mConnection.isAuthMethodAvailable(usuario,
				...
				if (f.exists()) {
					...
					SkStatus.logInfo("Autenticando with public key");
				...
				case Settings.bTUNNEL_TYPE_SSL_TLS:
					...
					try{
						if (customSNI != null && !customSNI.isEmpty() && !isProteger)
							SkStatus.logInfo("SNI/TLS: " + customSNI);
	...
	public void reconnectSSH() {
		...
		SkStatus.updateStateString(SkStatus.SSH_RECONECTANDO, "Reconecting..");
		...
		for (int i = 0; i < RECONNECT_TRIES; i++) {
			...
			if (!TunnelUtils.isNetworkOnline(mContext)) {
				...
				try {
					...
					SkStatus.logInfo("<font color='red'><strong>" + mContext.getString(R.string.state_disconnected) + "</font></strong>");
				}
	...
	protected void startTunnelVpnService() throws IOException {
		...
		if (m_dnsForward) {
			m_dnsResolvers = new String[]{mConfig.getVpnDnsResolverPrimary()};
			m_dnsResolvers = new String[]{mConfig.getVpnDnsResolverSecondary()};
		}
		else {
