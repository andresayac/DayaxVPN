...
public class Tunnel {
    ...
    private boolean startVpn(boolean forwardDns, String[] dnsResolver, String[] excludeIps,
        ...
        try {
            ...
            // still to test, subnet routing search "allow lan"
            if (enableTethering) {
                    ...
                    for (String app_pacote : filterApps) {
                        try {
                            if (filterBypassMode) {
                                ...
                                mHostService.onDiagnosticMessage(String.format("App Filter: VPN disabled for \"%s\"", app_pacote));
                            } else {
                                builder.addAllowedApplication(app_pacote);
                                mHostService.onDiagnosticMessage(String.format("App Filter: VPN enabled for \"%s\"", app_pacote));
                            }
                        } catch (PackageManager.NameNotFoundException e) {
                            mHostService.onDiagnosticMessage("Application \"" + app_pacote + "\" not found. App filter will not work, check settings.");
                        }
                    }
                }
            }
    ...
    private boolean routeThroughTunnel(final String socksServerAddress, final String[] dnsResolver, boolean forwardDns, final String udpResolver, final boolean transparentDns) {
        ...
        if (forwardDns) {
            ...
            String[] mServersDNS = dnsResolver;
            dnsgwRelay = String.format("%s:%d", mPrivateAddress.mIpAddress, pdnsdPort);

            mPdnsd = new Pdnsd(mHostService.getContext(), mServersDNS, DNS_RESOLVER_PORT,
